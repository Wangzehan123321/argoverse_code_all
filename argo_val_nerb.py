...
import torch
from model_argo_nerb import ArgoNet,init_weights
from argo_utils_nerb import ArgoverseDataset,MSE,ADE,FDE
...
#net.load_state_dict(torch.load('./SAIC/trained_models/argo_lstm_multi_fde_pretrain_SAIC/cslstm_31.tar'))
net.load_state_dict(torch.load('./SAIC/trained_models/argo_lstm_interaction_train/cslstm_99.tar'))
...
tsSet = ArgoverseDataset('../data/data_nerb/forecasting_features_val.pkl',interaction=True,val_metric=True,save_pkl=save_pkl)
...
with torch.no_grad():
    for i, data in enumerate(tsDataloader):
        ...
        if save_pkl:
            print(i)
            hist,fut,current_refer,seq_list,oracle_ct = data
        else:
            hist, current_pt, cum_start_idx, fut, current_refer, oracle_ct = data
        # Initialize Variables
        if args['use_cuda']:
            hist = hist.cuda()
            current_pt=current_pt.cuda()
            fut = fut.cuda()
        ...
        if args['use_maneuvers']:
            ...
            if save_pkl:
                for num in range(len(seq_list)):
                    traj_temp=[]
                    traj_temp.append(fut_1_abs[:, num, :])
                    traj_temp.append(fut_2_abs[:, num, :])
                    traj_temp.append(fut_3_abs[:, num, :])
            ...
            fut_pred = net(hist=hist, hist_pos=current_pt, seq_start_end=cum_start_idx)
...
if save_pkl:
    save_pkl_path="../save_pkl/argo_multi_ade.pkl"
    with open(save_pkl_path,"wb")as f:
        pkl.dump(traj_all_dict,f)
        print("pkl file has completed!")