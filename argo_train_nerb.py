...
from argo_utils_nerb import ArgoverseDataset,MSE
...
class Visualizer(object):
    ...
    def txt(self,name,text,**kwargs):
        ...
        self.index[name]=x+1
vis=Visualizer('argo_lstm_interaction_train')
...
trSet = ArgoverseDataset('../data/data_nerb/forecasting_features_train.pkl',interaction=True)#train datatset
valSet = ArgoverseDataset('../data/data_nerb/forecasting_features_val.pkl',interaction=True)#val dataset
...
for epoch_num in range(trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        st_time = time.time()
        hist_traj_batch, current_pt_batch, cum_start_idx, fut_batch = data
        ...
        if args['use_maneuvers']:
            ...
            fut_pred = net(hist=hist_traj_batch, hist_pos=current_pt_batch, seq_start_end=cum_start_idx)
    ...
    torch.save(net.state_dict(), './SAIC/trained_models/argo_lstm_interaction_train/cslstm_{}.tar'.format(str(epoch_num)))
