...
from SAIC_utils import ArgoverseDataset,MSE,ADE,FDE
...
tsSet = ArgoverseDataset('../data/forecasting_features_val.pkl',absolute=True,social_input=args["social_input"],vehicle_input=args["vehicle_input"],val_metric=True,save_pkl=True)
...
with torch.no_grad():
    for i, data in enumerate(tsDataloader):
        # st_time = time.time()
        # hist,hist_v,fut,current_refer,oracle_ct = data
        hist, fut, current_refer, seq_list = data
        ...
        if args['use_maneuvers']:
            ...
            fut_pred[:,:,:]=fut_pred[:,:,:]+current_refer.unsqueeze(0)
            ...
            for idx in range(len(seq_list)):
                # dict_save[seq_list[idx]]=fut_pred_abs[:,idx,:]
                dict_save[seq_list[idx]] = fut_pred[:, idx, :]
...
with open("./absolute_lstm.pkl","wb")as f:
    pkl.dump(dict_save,f)

# ##argodataset cv data
# import torch
# from SAIC_utils import ArgoverseDataset
# from torch.utils.data import DataLoader
# tsSet = ArgoverseDataset('../data/forecasting_features_val.pkl',absolute=True,val_metric=True,save_pkl=True)
# tsDataloader = DataLoader(tsSet,batch_size=1024,shuffle=True,num_workers=8,collate_fn=tsSet.collate_fn)
# dict_save={}
# with torch.no_grad():
#     for i, data in enumerate(tsDataloader):
#         # st_time = time.time()
#         hist,fut,hist_refer,seq_list = data
#         v_current=hist[-1,:,:]-hist[-2,:,:]
#         cv_fut_pred=torch.zeros_like(fut)
#         for i in range(fut.shape[0]):
#             cv_fut_pred[i,:,:]=hist[-1,:,:]+(i+1)*v_current
#         cv_fut_pred=cv_fut_pred+hist_refer
#         for idx in range(len(seq_list)):
#             dict_save[seq_list[idx]]=cv_fut_pred[:,idx,:]
#
# import pickle as pkl
# with open("./cv_fut_pred.pkl","wb")as f:
#     pkl.dump(dict_save,f)
