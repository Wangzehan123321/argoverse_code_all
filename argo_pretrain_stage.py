...
from SAIC_utils import ArgoverseDataset,MSE,maskedMSE
...
class Visualizer(object):
    ...
    def txt(self,name,text,**kwargs):
        ...
        self.index[name]=x+1
vis=Visualizer('argo_lstm_relative_speed_smoothl1_step')
...
trSet = ArgoverseDataset('../data/forecasting_features_train.pkl',social_input=args["social_input"],vehicle_input=args["vehicle_input"])#train datatset
valSet = ArgoverseDataset('../data/forecasting_features_val.pkl',social_input=args["social_input"],vehicle_input=args["vehicle_input"])#val dataset
...
for epoch_num in range(trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        st_time = time.time()
        hist_traj_batch, hist_v_batch, fut_batch = data
        ...
        if args['use_maneuvers']:
            ...
            #fut_pred = net(hist_traj_batch,hist_v_batch)
            fut_pred = net(hist_traj_batch, hist_v_batch)
            #l = maskedMSE(fut_pred, fut_batch,mask_mse)
            fut_pred=fut_pred[0:mask_seq,:,:]
            fut_batch=fut_batch[0:mask_seq,:,:]
            l = F.smooth_l1_loss(fut_pred, fut_batch)
    ...
    torch.save(net.state_dict(), './SAIC/trained_models/argo_lstm_relative_speed_smoothl1_step/cslstm_{}.tar'.format(str(epoch_num)))
