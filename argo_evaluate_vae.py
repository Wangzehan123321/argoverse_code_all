...
# net.load_state_dict(torch.load('./SAIC/trained_models/argo_lstm_relative_speed_vae_16_3/cslstm_99.tar'))
net.load_state_dict(torch.load('./SAIC/trained_models/argo_roadfeature_vae_16_3/cslstm_99.tar'))
...
tsSet = ArgoverseDataset('../data/forecasting_features_val.pkl',road_feature=args["road_feature"],social_input=args["social_input"],vehicle_input=args["vehicle_input"],val_metric=True,save_pkl=save_pkl)
...
with torch.no_grad():
    for i, data in enumerate(tsDataloader):
        ...
        if save_pkl:
            print(i)
            hist,hist_v,fut,current_refer,seq_list,oracle_ct = data
        else:
        ...
        if not args['vae_train']:
            #TODO:多模态实现
            fut_pred_all = net(hist,hist_v=hist_v,road_feature=road_feature)
